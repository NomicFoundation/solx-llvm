...
static uint64_t getFlag(const Module &M, StringRef Name) {
  auto *Value = mdconst::extract_or_null<ConstantInt>(M.getModuleFlag(Name));
  if (!Value)
    llvm_unreachable("Expected the module flag alongside the marker");
...
static bool foldMemoryGuard(Module &M) {
  ...
  if (!Decl->hasOneUser())
    llvm_unreachable("Expected a single memoryguard call");

  auto *Call = cast<CallInst>(*Decl->user_begin());
  uint64_t HeapStart =
      getFlag(M, EVMMemoryGuardFlag) + getFlag(M, EVMStackRegionSizeFlag);
