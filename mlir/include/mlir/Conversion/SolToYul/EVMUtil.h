...
private:
  ...
  /// Returns the helper function that clears a value of the identified struct
  /// \p structTy from storage, creating it (with its body) on first request.
  /// The helper is emitted as a `sol.func` in the enclosing contract so the
  /// existing contract/object machinery relocates, clones and lowers it like
  /// any other function. Its symbol is registered before the body is generated,
  /// so a self-reference reached while building the body (through the struct's
  /// dynamic-array members) lowers to a recursive call.
  mlir::sol::FuncOp getOrCreateClearStorageFn(mlir::sol::StructType structTy,
                                              mlir::Location loc);
...
public:
  ...
  /// Short, symbol-safe name for a data location, used as a helper name field.
  static const char *dataLocationName(mlir::sol::DataLocation loc);
