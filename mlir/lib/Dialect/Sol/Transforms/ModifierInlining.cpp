...
private:
  ...
  void emitLayer(OpBuilder &b, unsigned depth) {
    ...
    if (depth == invs.size()) {
      ...
      for (auto [param, arg] :
           llvm::zip_equal(srcBlks.front()->getArguments(), argsYield.getIns()))
    ...
    scope.walk([&](Operation *op) {
      TypeSwitch<Operation *>(op)
          .Case([&](sol::PlaceholderOp ph) { placeholders.push_back(ph); })
          .Case([&](sol::ReturnOp ret) { returns.push_back(ret); });
    });

    // A return exits the layer after writing the shared result slots. A
    // modifier-body `return` carries no operands and writes none, hence no
    // zip_equal.
...
struct ModifierInliningPass
  ...
  void runOnOperation() override {
    ...
    // Erasing mid-walk is safe: the walk is post-order, and every
    // sol.modifier_invocation referencing the modifier is already erased.
