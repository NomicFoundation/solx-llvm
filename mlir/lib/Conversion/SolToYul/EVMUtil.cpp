...
// Recursively clears the storage occupied by a value of type \p ty rooted at
// \p slot. See the header doc for the per-type handling. Shared by storage-array
// tail clearing (below) and `sol.delete` of a reference-typed storage variable.
void evm::Builder::genClearStorageValue(Type ty, Value slot, Location loc) {
  mlir::solgen::BuilderExt bExt(b, loc);

  // Zeros all storage slots occupied by a value of type \p ty at \p slot. The
  // leaf operation for scalars, packed types, and fixed-size arrays with
  // trivially-zeroable elements.
  ...
  if (auto structTy = dyn_cast<sol::StructType>(ty)) {
    ...
    // members (e.g. uint8 + bool) may share the same storage slot. A single
    // sstore(0) is sufficient for those to skip duplicates. Members with
    // dynamically-sized elements are never deduplicated; they always get their
    // own recursive call regardless of slot sharing.
...
void evm::Builder::genClearStorageArrayTail(Value arraySlot,
  ...
  }
  {
    ...
    // The per-value storage clear is evm::Builder::genClearStorageValue (above);
    // the tail loops below call it once per element / slot.
