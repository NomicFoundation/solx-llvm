  ...
  LogicalResult matchAndRewrite(yul::MemGuardOp op,
    ...
    // for a spill area.
    {
      ...
      r.create<LLVM::ModuleFlagsOp>(
          op.getLoc(),
          r.getArrayAttr(
              {LLVM::ModuleFlagAttr::get(
                   r.getContext(), LLVM::ModFlagBehavior::Error,
                   r.getStringAttr("evm-memory-guard"),
                   r.getI64IntegerAttr(guard.getValue().getZExtValue())),
               LLVM::ModuleFlagAttr::get(
                   r.getContext(), LLVM::ModFlagBehavior::Error,
                   r.getStringAttr("evm-stack-region-size"),
...
struct ScopeOpLowering : public OpRewritePattern<yul::ScopeOp> {
  ...
  LogicalResult matchAndRewrite(yul::ScopeOp op,
    ...
    for (Block &blk : op.getBodyRegion()) {
      if (auto yield = dyn_cast<yul::YieldOp>(blk.getTerminator())) {
        OpBuilder::InsertionGuard guard(r);
        r.setInsertionPoint(yield);
